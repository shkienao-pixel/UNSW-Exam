...
- **MCQ 答案预生成** — 提取真题时 AI 强制推断每道选择题的正确答案（有答案键用答案键，无则依靠知识推断），存入 `correct_answer` 字段；批改时全部走本地比对，彻底消除 MCQ 的 AI 调用
- **模型降级** — 简答批改（纯文字批量）从 `gpt-5.4` 降级至 `gpt-4o-mini`（快 3-5×，成本降 10×）；含图片答案批改从 `gpt-5.4` 降级至 `gpt-4o`（保留 Vision 能力）
- **效果**：提交一份纯选择题试卷批改延迟从 ~5s 降至 <100ms；混合题型试卷 AI 调用量大幅减少
...
**产品简化 & 架构清理：**
- **错题本改名** — `notes-and-mistakes` tab 改名为「错题本」，去除歧义
- **生成按钮归位** — 闪卡和模拟题 tab 加入「生成 · 100 ✦」按钮（含积分提示），生成完自动刷新内容；原 API 方法存在但无 UI 入口的问题修复
- **删除死路 OutputsTab** — 历史生成记录已内嵌各自 tab，移除无入口的独立历史页
- **exam_mock 迁入 Harness** — 移除 `_run_job_legacy` / `_payload_to_body`，所有生成类型走统一执行路径
- **充值管理后台** — Admin 新增「充值管理」Tab：订单流水（含用户邮箱）、统计卡片、手动补发积分（webhook 漏发补救）
- **Bug Fix** — 充值订单 ID 类型 UUID 与接口 int 声明不符导致手动补发 422，已修复
...
**AI 问答窗口 — 考试解析工作台：**
- **结构化答案卡片** — 每条 AI 回复以独立卡片展示，金色左边框 + "AI 解析" badge；支持完整 Markdown 渲染（标题/加粗/代码块/引用）
- **快捷操作 Chips** — 逐项分析 / 生成闪卡 / 出类似题 / 易错点总结，一键触发 `sendMessage()`
- **空状态引导** — 2×2 建议卡片替代空白页，降低冷启动门槛
- **操作栏** — 每条回复底部附 复制 / 收藏 按钮（UI 已就位）

**积分/解锁 UX 重设计：**
- **价值优先展示** — 锁定态改为模糊内容预览 + 底部细条解锁入口，弱化"付费墙"感
...
- 修复 FAB 按钮点击无响应（`onClick` 传空函数）
- 修复闪卡 header icon 与文字换行问题
- 修复笔记保存 403 + 浮窗支持拖拽 resize
- 修复 `FlashcardMistake.id` 类型错误（`string` → `number`）
...
**产品简化 & 架构清理：**
- **错题本改名** — 侧边栏 tab 由「笔记与错题」改为「错题本」
- **生成按钮归位** — 闪卡、模拟题 tab 加「生成 · 100 ✦」按钮，生成完自动刷新
- **删除死路 OutputsTab** — 历史记录内嵌各 tab，移除无入口的独立历史页
- **exam_mock 迁入 Harness** — 移除 `_run_job_legacy`，所有生成类型走统一路径
- **充值管理后台** — Admin 新增充值管理 Tab：订单流水 + 统计 + 手动补发
- **Bug Fix** — 手动补发接口 UUID/int 类型不匹配 422 已修复
