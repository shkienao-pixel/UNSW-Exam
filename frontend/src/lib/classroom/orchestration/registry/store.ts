...
import { USER_AVATAR } from '@/lib/classroom/types/roundtable';
import type { Participant, ParticipantRole } from '@/lib/classroom/types/roundtable';
import { useUserProfileStore } from '@/lib/classroom/store/user-profile';
...
export async function loadGeneratedAgentsForStage(stageId: string): Promise<string[]> {
  const { getGeneratedAgentsByStageId } = await import('@/lib/classroom/utils/database');
...
export async function saveGeneratedAgents(
  ...
  const { db } = await import('@/lib/classroom/utils/database');
