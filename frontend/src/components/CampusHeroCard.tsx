...
export default function CampusHeroCard({ onGuestLogin, guestLoading }: Props) {
  ...
  return (
    <div className="relative w-full max-w-3xl mx-auto mt-8 mb-4 fade-in-up px-4 sm:px-0"
      ...
      <div className="relative rounded-3xl overflow-hidden flex flex-col sm:flex-row"
        ...
        {/* ── Right panel: campus photo + CTA ── */}
        <div className="relative sm:w-64 flex-shrink-0 overflow-hidden"
          style={{ minHeight: 240 }}>
          ...
          {/* Overlay gradient — left edge fades into left panel */}
        ...
        <div className="absolute bottom-2 left-0 right-0 flex justify-center pointer-events-none sm:pointer-events-auto"
          style={{ zIndex: 20 }}>
          <button
            ...
            {guestLoading
              ? '进入中…'
              : <>New! Click the rainbow steps to explore resources. <span style={{ color: '#333344' }}>（新：点击彩虹阶梯探索资源。）</span></>
            }
          </button>
        </div>

        {/* Extra bottom padding for tooltip */}
        <div className="absolute bottom-0 left-0 right-0" style={{ height: 44, pointerEvents: 'none' }} />
      </div>
